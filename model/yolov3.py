...
def yolo_boxes(pred, anchors, num_class):
...
def output_bbox(input, yolo_iou_threshold, yolo_score_threshold):
    b, c, t = [], [], []
...
def yolo_loss(anchors, num_class, ignore_thresh=0.5):
    def yolo_loss(y_true, y_pred):
        # 1. transform all pred outputs
        # y_pred: (batch_size, grid, grid, anchors, (x, y, w, h, obj, ...cls))
        pred_box, pred_obj, pred_class, pred_xywh = yolo_boxes(
            y_pred, anchors, num_class)
...
class YoloV3(tf.keras.Model):
    ...
    def call(self, inputs, training=False, **kwargs):
        ...
        boxes_1 = yolo_boxes(scale_1_detector, anchors[masks[0]], self.num_class)
        boxes_2 = yolo_boxes(scale_2_detector, anchors[masks[1]], self.num_class)
        boxes_3 = yolo_boxes(scale_3_detector, anchors[masks[2]], self.num_class)

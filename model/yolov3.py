...
class YoloV3(tf.keras.Model):
    yolo_anchors = np.array([(5, 3), (6, 6), (11, 4), (8, 13), (13, 8), (21, 6), (19, 18), (34, 11), (56, 28)],
                            np.float32) / 416
    ...
    def output_bbox(self, input, yolo_iou_threshold, yolo_score_threshold):
        b, c, t = [], [], []
    ...
    def loss_layer(self, y_pred, y_true, anchors):
    ...
    def yolo_loss(self, pred_sbbox, pred_mbbox, pred_lbbox, true_sbbox, true_mbbox, true_lbbox):
        anchors = YoloV3.yolo_anchors
        masks = YoloV3.yolo_anchor_masks
        loss_sbbox = self.loss_layer(pred_sbbox, true_sbbox, anchors[masks[0]])
        loss_mbbox = self.loss_layer(pred_mbbox, true_mbbox, anchors[masks[1]])
        loss_lbbox = self.loss_layer(pred_lbbox, true_lbbox, anchors[masks[2]])
    ...
    def call(self, inputs, training=False, **kwargs):
        ...
        boxes_1 = self.yolo_boxes(scale_1_detector, anchors[masks[0]], self.num_class)
        boxes_2 = self.yolo_boxes(scale_2_detector, anchors[masks[1]], self.num_class)
        boxes_3 = self.yolo_boxes(scale_3_detector, anchors[masks[2]], self.num_class)

        output = self.output_bbox(
            input=(boxes_1[:3], boxes_2[:3], boxes_3[:3]),
            yolo_iou_threshold=self.yolo_iou_threshold,
            yolo_score_threshold=self.yolo_score_threshold
        )
